...
class AuthItem extends Model
{
    ...
    public function getItems()
    {
        ...
        foreach (array_keys($manager->getPermissions()) as $name) {
            $available[$name] = $name[0] == Configs::instance()->routePrefix ? 'route' : 'permission';  ///[yii2-brainbase v0.4.4 (admin:fix# mdmsoft/yii2-admin multi app)]
        }

        $assigned = [];
        foreach ($manager->getChildren($this->_item->name) as $item) {
            $assigned[$item->name] = $item->type == 1 ? 'role' : ($item->name[0] == Configs::instance()->routePrefix ? 'route' : 'permission'); ///[yii2-brainbase v0.4.4 (admin:fix# mdmsoft/yii2-admin multi app)]
